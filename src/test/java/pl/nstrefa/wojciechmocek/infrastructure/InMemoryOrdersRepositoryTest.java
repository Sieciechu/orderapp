...
//        List<Product> secondShopping = Arrays.asList(new Product("butter", 1, 3.0));


        // when
        repository.store(new Order("1", 1, firstShopping));


        // then
        Set<Order> orders = repository.getAll();
        Assertions.assertEquals(1, orders.size());
        Order expectedOrder = new Order("1", 1, Arrays.asList(
            new Product("bread", 1, 2.5),
            new Product("chocolate", 2, 1.5)
        ));
